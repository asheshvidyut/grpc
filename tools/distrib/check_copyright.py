...
def replace_copyright(license_text, file_text, filename):
    m = re.search(RE_C_STYLE_COMMENT_LICENSE, file_text)
    if m:
...
def check_file(filename):
    ...
    if filename.startswith("py_xds_protos") and (
    ...
    if filename.startswith(_ENFORCE_CPP_STYLE_COMMENT_PATH_PREFIX) and ext in [
        ...
        messages.append((args.skips, "skip", filename))
        return True, messages
    try:
        text = load(filename)
    except:
    ...
    if m:
        ...
        if args.fix:
            write_copyright(license_text, text, filename)
            messages.append((1, "copyright missing (fixed)", filename))
        else:
            messages.append((1, "copyright missing", filename))
        ok = False
    return ok, messages
...
if __name__ == "__main__":
    ...
    with Pool() as pool:
        results = pool.map(check_file, filename_list)

    for file_ok, messages in results:
        if not file_ok:
            ok = False
        for cond, why, fname in messages:
            log(cond, why, fname)
