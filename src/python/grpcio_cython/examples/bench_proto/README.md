...
This example exists to answer one question: **how much of the grpcio_cython
...
| `native` | grpcio_cython handler | C | **no** | **yes (entire handler)** |
...
shows what grpcio_cython adds on top.
...
ctypes` — the gap that's specifically attributable to grpcio_cython.
...
- If `ctypes` is close to `native`, the workload is so trivial that
  framework overhead dominates. This is the "don't use grpcio_cython here"
  signal (ctypes overhead ~5 μs/call, native ~3–5 μs better with the Cython
  fast path).

## What this benchmark does NOT measure

- **AIO server.** grpcio_cython MVP is sync-only; the asyncio path is the
