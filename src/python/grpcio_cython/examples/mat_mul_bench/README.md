...
3. **Native JIT (`--jit` / default):** High-performance, zero-copy native JIT-compiled Cython/C++ handler using `grpcio_cython` where both the matrix multiplication and the Protobuf serialization/deserialization are executed entirely in C++.
