...
Include `grpcio_cython/handler.hpp` and use one macro per RPC type. The
macro generates the C ABI entry point; you write only typed business logic.

```cpp
#include "grpcio_cython/handler.hpp"
#include "ranker.pb.h"
...
library with C-linkage exports, include `grpcio_cython/handler.h`
and target the raw struct-based ABI. The C++ macros expand to exactly this
shape — see [include/grpcio_cython/handler.h](include/grpcio_cython/handler.h):
...
module = grpcio_cython.load_native_module("./my_handler.so")
...
The C ABI is in `include/grpcio_cython/handler.h`, currently version 2.
Stability rules:

  - `GRPCIO_NATIVE_ABI_VERSION` is bumped on incompatible changes.
  - Every handler library must export `grpcio_cython_abi_version()`; the
...
```
   ...
   client RPC -------> | grpc C-core (Cython glue) |
                ...
                request_deserializer is None: bytes pass through
                                   ...
                                   v
                       +---------------------------+
                       | NativeUnaryUnaryBehavior  |   <-- grpcio_cython
                       +-----------+---------------+
                                   |
...
branch, `grpcio_cython` automatically uses a Cython entry point that skips
...
```python
handler = grpcio_cython.native_stream_stream_rpc_method_handler(
    module, "RunningSum"
)
```
