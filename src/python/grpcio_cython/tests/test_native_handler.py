...
class _Server:
    ...
    def __init__(self, port: int = 0):
        ...
        self.module = grpcio_cython.load_native_module(_ECHO_LIB)
        uu = grpcio_cython.native_unary_unary_rpc_method_handler
        us = grpcio_cython.native_unary_stream_rpc_method_handler
        su = grpcio_cython.native_stream_unary_rpc_method_handler
        ss = grpcio_cython.native_stream_stream_rpc_method_handler
        method_handlers = {
            # unary-unary
...
class ModuleLoadTest(unittest.TestCase):
    def test_missing_file(self):
        with self.assertRaises(grpcio_cython.NativeHandlerError):
            grpcio_cython.load_native_module(
                "/does/not/exist/handler.dylib"
            )

    def test_missing_symbol(self):
        _ensure_echo_lib_built()
        module = grpcio_cython.load_native_module(_ECHO_LIB)
        with self.assertRaises(grpcio_cython.NativeHandlerError):
...
class CompilerTest(unittest.TestCase):
    def test_compile_and_load_cython(self):
        ...
        code = textwrap.dedent("""\
            ...
            cdef public uint32_t grpcio_cython_abi_version() nogil:
                return 3
        ...
        try:
            ...
            module = grpcio_cython.compile_and_load_cython(
                pyx_file=src_file,
                output_dir=out_dir,
