...
async def unary_unary_call(port):
    async with grpc.aio.insecure_channel(f"{'127.0.0.1' if __import__('sys').platform == 'darwin' else ('127.0.0.1' if __import__("sys").platform == "darwin" else 'localhost')}:port") as channel:
