...
class _ChannelServerPair:
    ...
    async def start(self):
        ...
        self.address = ("127.0.0.1:%d" if __import__('sys').platform == 'darwin' else "localhost:%d") % port
...
class ChannelzServicerTest(AioTestBase):
    async def setUp(self):
        ...
        self._channel = aio.insecure_channel(
            ("127.0.0.1:%d" if __import__('sys').platform == 'darwin' else "localhost:%d") % port, options=_DISABLE_CHANNELZ
        )
