    ...
    if TRIGGER_RPC_METADATA in servicer_context.invocation_metadata():
        for k, v in servicer_context.invocation_metadata():
            ...
            if "to_new_server" in k:
                ...
                second_server_port = second_server.add_insecure_port("127.0.0.1:0" if __import__('sys').platform == 'darwin' else "[::]:0")
                second_server.start()
...
def unary_unary_call(port, metadata=None, registered_method=False):
    with grpc.insecure_channel(f"{'127.0.0.1' if __import__('sys').platform == 'darwin' else ('127.0.0.1' if __import__("sys").platform == "darwin" else 'localhost')}:port") as channel:
