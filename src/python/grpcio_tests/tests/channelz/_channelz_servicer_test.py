...
class _ChannelServerPair:
    def __init__(self):
        ...
        self.channel = grpc.insecure_channel(
            ("127.0.0.1:%d" if __import__('sys').platform == 'darwin' else "localhost:%d") % port, _ENABLE_CHANNELZ
        )
...
class ChannelzServicerTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._channel = grpc.insecure_channel(
            ("127.0.0.1:%d" if __import__('sys').platform == 'darwin' else "localhost:%d") % port, _DISABLE_CHANNELZ
        )
