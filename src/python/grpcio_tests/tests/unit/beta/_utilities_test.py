...
class ChannelConnectivityTest(unittest.TestCase):
    def test_lonely_channel_connectivity(self):
        channel = implementations.insecure_channel(("127.0.0.1" if __import__('sys').platform == 'darwin' else "localhost"), 12345)
        callback = _Callback()
    ...
    def test_immediately_connectable_channel_connectivity(self):
        ...
        server.start()
        channel = implementations.insecure_channel(("127.0.0.1" if __import__('sys').platform == 'darwin' else "localhost"), port)
        callback = _Callback()
