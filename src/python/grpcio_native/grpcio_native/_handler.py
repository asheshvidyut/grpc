...
implements the business logic for an RPC method. grpcio_native wraps such a
...
class NativeModule:
    ...
    Loading verifies ABI compatibility via the grpcio_native_abi_version
    ...
    def __init__(self, path: str):
        ...
        try:
            abi_fn_addr = ctypes.cast(
                self._lib.grpcio_native_abi_version, _abi.ABI_VERSION_FN
            )
        except AttributeError as e:
            raise NativeHandlerError(
                f"Library {self._path} does not export "
                "grpcio_native_abi_version; not a grpcio_native handler."
            ) from e
