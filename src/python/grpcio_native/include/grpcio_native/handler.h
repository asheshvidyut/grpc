 ...
 * path. The grpcio_native dispatcher calls the function with the Python GIL
 ...
 *     grpcio_native_abi_version(). A mismatch is a load-time error.
 */
 ...
 * checks this matches the version expected by the running grpcio_native
 * Python package. If it doesn't, loading fails.
 */
typedef uint32_t (*grpcio_native_abi_version_fn)(void);
...
#define GRPCIO_NATIVE_DECLARE_ABI()                                  \
  ...
  grpcio_native_abi_version(void) {                                  \
    return GRPCIO_NATIVE_ABI_VERSION;                                \
  }
#else
#define GRPCIO_NATIVE_DECLARE_ABI()                                  \
  GRPCIO_NATIVE_EXPORT uint32_t grpcio_native_abi_version(void) {    \
    return GRPCIO_NATIVE_ABI_VERSION;                                \
  }
#endif
