...
# Resolve path to grpcio_native
sys.path.insert(0, os.path.abspath(os.path.join(_GEN_DIR, "..", "..")))
import grpcio_native

class LoggingInterceptor(grpc.ServerInterceptor):
    def intercept_service(self, continuation, handler_call_details):
        print(f"\n[LoggingInterceptor] Intercepted RPC call to: {handler_call_details.method}")
...
def main():
    ...
    # 2. Load the C++ shared library via grpcio_native
    ...
    echo_unary_handler = grpcio_native.native_unary_unary_rpc_method_handler(
        module, "echo_unary"
    )
    echo_stream_handler = grpcio_native.native_unary_stream_rpc_method_handler(
        module, "echo_stream"
    )

    # 4. Set up standard Python gRPC server with the LoggingInterceptor
    server = grpc.server(
